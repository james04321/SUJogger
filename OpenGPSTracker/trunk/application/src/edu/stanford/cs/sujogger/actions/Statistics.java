...
import edu.stanford.cs.sujogger.actions.utils.GraphCanvas;
 ...
 * 
 * @version $Id: Statistics.java 465 2010-03-22 19:10:42Z rcgroot $
...
public class Statistics extends Activity {
	...
	protected void onCreate(Bundle load) {
		...
		avgSpeedView = (TextView) findViewById(R.id.stat_avgspeed);
		paceView = (TextView) findViewById(R.id.stat_pace);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		menu.add(ContextMenu.NONE, MENU_GRAPHTYPE, ContextMenu.NONE, R.string.menu_graphtype)
				.setIcon(R.drawable.ic_menu_picture).setAlphabeticShortcut('t');
		menu.add(ContextMenu.NONE, MENU_TRACKLIST, ContextMenu.NONE, R.string.menu_tracklist)
				.setIcon(R.drawable.ic_menu_show_list).setAlphabeticShortcut('l');
		menu.add(ContextMenu.NONE, MENU_SHARE, ContextMenu.NONE, R.string.menu_shareTrack).setIcon(
				R.drawable.ic_menu_share).setAlphabeticShortcut('s');
		return result;
	}
	...
	protected Dialog onCreateDialog(int id) {
		...
		case DIALOG_GRAPHTYPE:
			...
			builder.setTitle(R.string.dialog_graphtype_title).setIcon(
	...
	private void drawTrackingStatistics() {
		...
		try {
			waypointsCursor = resolver.query(Uri.withAppendedPath(mTrackUri, "waypoints"),
					new String[] { "max  (" + Waypoints.TABLE + "." + Waypoints.SPEED + ")",
							...
							"count(" + Waypoints.TABLE + "." + Waypoints._ID + ")" }, null, null,
					null);
		...
		try {
			trackCursor = resolver.query(mTrackUri, new String[] { Tracks.NAME, Tracks.DURATION, Tracks.USER_ID }, null, null, null);
			if (trackCursor.moveToLast()) {
				tracknameText = trackCursor.getString(0);
				trackDuration = trackCursor.getLong(1);
				trackOwnerId = trackCursor.getInt(2);
			}
		}
		finally {
			if (trackCursor != null) {
		...
		mGraphView1.setData(mTrackUri, starttime, endtime, distanceTraveled, minAltitude,
				maxAltitude, maxSpeeddb, mUnits);
		mGraphView2.setData(mTrackUri, starttime, endtime, distanceTraveled, minAltitude,
				maxAltitude, maxSpeeddb, mUnits);
		mGraphView3.setData(mTrackUri, starttime, endtime, distanceTraveled, minAltitude,
				maxAltitude, maxSpeeddb, mUnits);
		mGraphView4.setData(mTrackUri, starttime, endtime, distanceTraveled, minAltitude,
				maxAltitude, maxSpeeddb, mUnits);

		double avgSpeedfl = mUnits.conversionFromMeterAndMiliseconds(distanceTraveled, duration);
		double distanceTraveledInMeters = mUnits.conversionFromMeter(distanceTraveled);
		avgSpeedText = String.format("%.2f %s", avgSpeedfl, mUnits.getSpeedUnit());
		distanceText = String.format("%.2f %s", distanceTraveledInMeters, mUnits.getDistanceUnit());
		
		
		
		if (trackOwnerId == Common.getRegisteredUser(this).id) {
			avgSpeedView.setText(avgSpeedText);
			timeView.setText(Common.durationString(this, duration));
			paceView.setText(Common.paceString(this, duration, distanceTraveled));
		}
		else {
			Common.log(TAG, "trackDuration=" + trackDuration + "; distance=" + distanceTraveled);
			avgSpeedView.setText(Common.speedString(this, distanceTraveled/trackDuration));
			timeView.setText(Common.durationString(this, trackDuration));
			paceView.setText(Common.paceString(this, trackDuration, distanceTraveled));
		}
